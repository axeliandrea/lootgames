...
def make_keyboard(menu_key: str, user_id=None, page: int = 0) -> InlineKeyboardMarkup:
    ...
    # --- POIN PRIBADI (BB) - tampilkan info ---
        ...
        username = pdata.get("username", None) or None
        text_line = f"📊 Poin Pribadi\n\nUsername: @{username if username else ( 'user'+str(user_id) )}\nPoints: {pts} pts\nLevel: {lvl} {yapping.get_badge(lvl)}"
        buttons.append([InlineKeyboardButton("⬅️ Kembali", callback_data="B")])
        # return keyboard but caller should edit message text with text_line
        return InlineKeyboardMarkup(buttons), text_line
...
async def send_single_emoji(client: Client, chat_id: int, emoji: dict, text: str = "", reply_to: int = None):
    # Build a text containing a custom emoji at start (Telegram requires entity)
    dummy = "⬛"
...
async def callback_handler(client: Client, callback_query: CallbackQuery):
    ...
    if data.startswith("FISH_CONFIRM_"):
        ...
        # kirim animasi / notifikasi ke TARGET_GROUP
        sent = await send_single_emoji(client, TARGET_GROUP, FISHING_EMOJI, f" @{username} sedang melempar umpan ({jenis})..")
        try:
        ...
        async def fishing_task():
            try:
                ...
                # jika ada loot engine, panggil dan kirim hasil
                loot_text = None
                if fishing_loot:
                    try:
                        loot_result = await fishing_loot(client, None, username, user_id, umpan_type=jenis)
                        loot_text = f"🎣 @{username} mendapatkan {loot_result}!"
                    except Exception as e:
                        logger.error(f"Error fishing_loot: {e}")
                        loot_text = f"🎣 @{username} mendapatkan sesuatu (error pada loot engine)."
                else:
                    loot_text = f"🎣 @{username} berhasil mendapatkan ikan!"
                await asyncio.sleep(10)
                await send_single_emoji(client, TARGET_GROUP, CATCH_EMOJI, f" {loot_text}", reply_to=sent.id if sent else None)
                # jika fishing gives rewards stored or points, handle here (opsional)
            except Exception as e:
    ...
    # --- TRANSFER INITIATION ---
    if data in ("TRANSFER_COMMON_OK","TRANSFER_RARE_OK","TRANSFER_LEGEND_OK","TRANSFER_MYTHIC_OK"):
        map_cb = {
            "TRANSFER_COMMON_OK":"A",
            "TRANSFER_RARE_OK":"B",
            "TRANSFER_LEGEND_OK":"C",
            "TRANSFER_MYTHIC_OK":"D"
        }
        jenis = map_cb.get(data,"A")
        TRANSFER_STATE[user_id] = {"jenis": jenis}
        await callback_query.message.reply("Masukkan target dan jumlah. Contoh: @username 1")
        return

    # --- TUKAR_POINT INIT ---
    if data == "TUKAR_POINT":
        TUKAR_POINT_STATE[user_id] = {"step":1,"jumlah_umpan":0}
        await callback_query.message.reply("Masukkan jumlah umpan yang ingin ditukar:")
        return
    ...
    if data == "TUKAR_CONFIRM":
        ...
        if not info or info.get("step") != 2:
            await callback_query.answer("❌ Proses tidak valid.", show_alert=True)
            return
        jml = info["jumlah_umpan"]
        points = yapping.load_points()
        user_pts = points.get(str(user_id),{}).get("points",0)
        if user_pts < jml*100:
            await callback_query.answer("❌ Point tidak cukup.", show_alert=True)
            TUKAR_POINT_STATE.pop(user_id,None)
            return
        # lakukan tukar
        yapping.update_points(user_id, -jml*100)
        umpan.add_umpan(user_id, "A", jml)
        await callback_query.message.reply(f"✅ Tukar berhasil! {jml} umpan ditambahkan ke akunmu.")
        TUKAR_POINT_STATE.pop(user_id,None)
        return

    # --- GENERIC MENU NAVIGATION ---
    if data in MENU_STRUCTURE:
        # special case BB (poin pribadi) - kita ingin menampilkan info + keyboard
        if data == "BB":
            kb_and_text = make_keyboard("BB", user_id)
            if isinstance(kb_and_text, tuple):
                kb, text = kb_and_text
                try:
        ...
        try:
            ...
            # make_keyboard can return (kb, text) only for BB; otherwise InlineKeyboardMarkup
            if isinstance(kb, tuple):
                kb = kb[0]
            await callback_query.message.edit_text(MENU_STRUCTURE[data]["title"], reply_markup=kb)
        except Exception as e:
            logger.error(f"Gagal edit message menu nav: {e}")
        return

    # fallback: unknown callback - just acknowledge
    await callback_query.answer()
    ...
    if TRANSFER_STATE.get(user_id):
        try:
            ...
            if user_id == OWNER_ID:
                ...
                if sender_data.get(jenis,{}).get("umpan",0) < amount:
...
async def show_leaderboard(callback_query: CallbackQuery, user_id: int, page: int = 0):
    ...
    try:
        ...
        try:
            await callback_query.message.reply(text, reply_markup=make_keyboard("BBB", user_id, page))
        except Exception as e:
            logger.error(f"Gagal tampilkan leaderboard: {e}")
