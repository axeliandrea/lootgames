...
# ============================================================
# 🎯 HANDLER 1: MENU TUKAR COIN (D2C)
# ============================================================
@Client.on_callback_query(filters.regex("^D2C"))
async def handle_d2c(client, cq):
    data = cq.data
    uid = cq.from_user.id

    if data == "D2C":
        kb = InlineKeyboardMarkup([
            [InlineKeyboardButton("🐛 Common Type A", callback_data="D2C_COMMON_A")],
            [InlineKeyboardButton("🪱 Rare Type B", callback_data="D2C_COMMON_B")],
            [InlineKeyboardButton("⬅️ Kembali", callback_data="E")]
        ])
        ...
        await cq.message.edit_text(
            ...
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ Batal", callback_data="D2C")]])
        )
        return
...
# ============================================================
# 🎣 HANDLER 2: FISHING SYSTEM (TERMASUK AUTO CATCH)
# ============================================================
async def fishing_task(client, uname, user_id, jenis, task_id):
    try:
        ...
        logger.error(f"[FISHING TASK] Error @{uname}: {e}")


@Client.on_callback_query(filters.regex("^FISH_CONFIRM_"))
async def handle_fish_confirm(client, cq):
    data = cq.data
    user_id = cq.from_user.id
    uname = cq.from_user.username or f"user{user_id}"
    ...
    if now - last_time < 10:
        await cq.answer("⏳ Tunggu 10 detik sebelum memancing lagi!", show_alert=True)
        return
    ...
    # Menu baru dengan pilihan auto catch
    kb = InlineKeyboardMarkup([
        [InlineKeyboardButton("🎣 Catch lagi", callback_data=f"FISH_CONFIRM_{jenis}")],
        [
            InlineKeyboardButton("🤖 Auto 5x", callback_data=f"AUTO_FISH_{jenis}_5"),
            InlineKeyboardButton("🤖 Auto 10x", callback_data=f"AUTO_FISH_{jenis}_10"),
        ],
        [
            InlineKeyboardButton("🤖 Auto 50x", callback_data=f"AUTO_FISH_{jenis}_50"),
            InlineKeyboardButton("❌ Cancel Auto", callback_data=f"AUTO_FISH_CANCEL_{jenis}"),
        ],
...
# 🤖 AUTO CATCH HANDLER
# ============================================================
@Client.on_callback_query(filters.regex("^AUTO_FISH_"))
async def handle_auto_fish(client, cq):
    data = cq.data
    user_id = cq.from_user.id
    uname = cq.from_user.username or f"user{user_id}"

    if data.startswith("AUTO_FISH_CANCEL_"):
        if user_id in AUTO_FISHING_STATE:
            AUTO_FISHING_STATE[user_id]["cancel"] = True
            await cq.answer("🛑 Auto fishing dibatalkan!", show_alert=True)
        else:
            await cq.answer("❌ Tidak ada auto fishing yang aktif.", show_alert=True)
        return

    # parsing format: AUTO_FISH_COMMON_10 → jenis=COMMON, count=10
    parts = data.split("_")
    jenis = parts[2]
    count = int(parts[3]) if len(parts) > 3 else 50

    if AUTO_FISHING_STATE.get(user_id, {}).get("active", False):
        await cq.answer("⏳ Kamu sudah sedang auto fishing!", show_alert=True)
        return

    AUTO_FISHING_STATE[user_id] = {"active": True, "cancel": False}

    await cq.answer(f"🤖 Auto Catch {count}x dimulai!")

    async def auto_fishing():
        for i in range(count):
            state = AUTO_FISHING_STATE.get(user_id, {})
            if state.get("cancel"):
                await client.send_message(user_id, f"🛑 Auto fishing dihentikan ({i}/{count})")
                break
            ...
            if user_id != OWNER_ID:
                ud = umpan.get_user(user_id)
                if not ud or ud.get(jk, {}).get("umpan", 0) <= 0:
                    await client.send_message(user_id, "❌ Umpanmu habis, auto fishing berhenti.")
                    break
        ...
        AUTO_FISHING_STATE[user_id] = {"active": False, "cancel": False}
        await client.send_message(user_id, "✅ Auto fishing selesai!")
